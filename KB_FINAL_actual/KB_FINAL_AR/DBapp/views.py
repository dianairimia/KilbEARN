        ...
        for i in Main.objects.all():
            ...
            if x[0] == username and x[1]== password:
                ...
                for p in User.objects.all():
                    if username==str(p):
                        login(request,p)
                        finishedlogin = render(request, 'DBapp/mainmenu.html')
                        global uname
                        uname=username
                        finishedlogin.set_cookie(key='uname', value=request.user.username)
...
def chat(request):
    print(request.session)
    val_dict = {'insert_val':"This can be modified with python, in the views.py file in DBapp" }
    return render(request, 'DBapp/chat.html', context=val_dict)
...
def room(request):
    ...
    if request.method=="GET":
        print("TESTING")
    return render(request, 'DBapp/room.html', {
        'room_name': 'kbconnect',
        'uname' : uname
    })
